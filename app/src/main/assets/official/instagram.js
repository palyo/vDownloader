...
function getArticlePostUrl(article) {
    // Find the anchor tag with the href containing /p/ within the article
...
function removeLastPart(url) {
    ...
    if (url.endsWith('/')) {
        // Remove the last part by finding the position of the second to last slash
        var lastSlashIndex = url.lastIndexOf('/', url.length - 2);
        // Return the URL up to and including the second to last slash
        return url.substring(0, lastSlashIndex + 1);
    }
    return url; // If the URL does not end with a slash, return it as is
}